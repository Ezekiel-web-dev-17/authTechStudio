...
export const updateViews = async (req, res, next) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "Views updated successfully", post });
  } catch (error) {
    next(error);
  }
};
...
export const updatePost = async (req, res, next) => {
  try {
    ...
    if (
      existingPost.by.toString() !== req.user._id.toString() &&
      !req.user.isAdmin
    ) {
...
export const deletePostByAdmin = async (req, res, next) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "Post deleted successfully by Admin." });
  } catch (error) {
    next(error);
  }
};
